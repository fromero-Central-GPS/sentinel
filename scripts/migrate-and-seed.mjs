...
const DATABASE_URL = 'postgresql://neondb_owner:npg_BmGQUvX3ZI4R@ep-long-cloud-atc2j966.c-9.us-east-1.aws.neon.tech/neondb?sslmode=require';
const sql = neon(DATABASE_URL);

async function run() {
  // Use tagged-template for all queries
  const tables = await sql`SELECT table_name FROM information_schema.tables WHERE table_catalog = ${'neondb'} AND table_schema = ${'public'} AND table_type = ${'BASE TABLE'} ORDER BY table_name`;
  console.log('Tables result type:', typeof tables, Array.isArray(tables), tables ? tables.length : 'null');
  ...
  // Seed
  const plans = [
    { name: 'Free', slug: 'free', price: '0', maxUsers: '1', maxConvs: '100', forense: 'true', live: 'false', won: 'false' },
    { name: 'Pro', slug: 'pro', price: '49900', maxUsers: '5', maxConvs: '5000', forense: 'true', live: 'true', won: 'true' },
    { name: 'Enterprise', slug: 'enterprise', price: '149900', maxUsers: '999', maxConvs: '999999', forense: 'true', live: 'true', won: 'true' },
  ];
