...
// ─── Helper: ejecutar pipeline de análisis forense ─────────────────────────
...
function runForensicsPipeline(
  ...
  return generateBatchSummary(analyses, opps[0]?.pipelineId ?? 'unknown', 'Sentinel');
}
...
// ─── Helper: llamar GHL API con credenciales del tenant ────────────────────

async function fetchGhlLostOpportunities(
  token: string,
  locationId: string,
  limit = 20,
) {
  const url = `https://services.leadconnectorhq.com/opportunities/search?location_id=${locationId}&status=lost&limit=${limit}`;
...
async function fetchGhlConversationMessages(
  ...
  const url = `https://services.leadconnectorhq.com/conversations/${conversationId}/messages?limit=50`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${token}`, Version: '2021-07-28' },
  });
  if (!res.ok) return [];
  const data = await res.json() as { messages?: Array<{
    id: string; direction: string; body: string; messageType: string; dateAdded: string;
  }> };
...
export async function GET(request: Request) {
  ...
  // ─── Modo mock: datos demo sin credenciales ───────────────────────────

  if (mode === 'mock') {
    const conversations = buildMockConversations();
    const opps = [
      { id: 'OPP-1', name: conversations[0].contactName, contactId: 'c1', contactName: conversations[0].contactName, monetaryValue: 41116655, pipelineId: 'tenant-pipeline', pipelineStageId: 'lost', pipelineStageName: 'Perdido', status: 'lost' as const, createdAt: '2026-01-15T00:00:00Z' },
      { id: 'OPP-2', name: conversations[1].contactName, contactId: 'c2', contactName: conversations[1].contactName, monetaryValue: 4300000, pipelineId: 'tenant-pipeline', pipelineStageId: 'lost', pipelineStageName: 'Perdido', status: 'lost' as const, createdAt: '2026-02-20T00:00:00Z' },
    ];
  ...
  // ─── Modo live: datos reales via GHL API con credenciales del tenant ──
  ...
  if (mode === 'live') {
    ...
    try {
      const rawOpps = await fetchGhlLostOpportunities(token, locationId, 20) as Array<{
        id: string; name: string; contact?: { id: string; name: string };
        monetaryValue?: number; pipelineId?: string; pipelineStageId?: string;
        pipelineStageName?: string; status: string; createdAt?: string;
        conversationId?: string;
      }>;
      ...
      const opps = rawOpps.slice(0, 15).map((opp) => ({
        id: opp.id,
        name: opp.name,
        contactId: opp.contact?.id ?? opp.id,
        contactName: opp.contact?.name ?? opp.name,
